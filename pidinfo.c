...
static int display_tracked_processes(struct seq_file *output, void *v) {
    ...
    for (index = 0; index < tracked_pid_count; index++) {
        ...
        seq_printf(output, "process_name = [%s]\tpid = [%d]\tstate = [%u]\n", process_task->comm, process_task->pid, READ_ONCE(process_task->__state));
    }
...
static ssize_t write_pid_to_proc_file(struct file *file, const char __user *user_buffer, size_t buffer_length, loff_t *offset) {
